...
import { listenFolderDrop, type DropPoint } from './tauri';
...
class TerminalManager {
  ...
  acquire(sessionId: string): TerminalInstance {
    ...
    // Unicode 11 widths. Without this, emoji and CJK glyphs are measured one
    // cell wide and every subsequent column on the line is misaligned.
    try {
    ...
    // Make URLs in output clickable, opened by the host (new tab / browser).
    try {
  ...
  private setupDragDrop(sessionId: string, container: HTMLElement): void {
    ...
    // In the desktop app the OS hands us real absolute paths, so a dropped
    // folder needs no guessing. The browser DataTransfer path above can only
    // see a folder's *name* and has to resolve it server-side, which picks the
    // wrong directory whenever the name isn't unique.
    // Tauri drag events are window-wide, so with split panes every terminal
    // hears them. Only the pane under the cursor should react.
    const isUnderCursor = (pos?: DropPoint) => {
      if (!pos) return true; // no position reported — treat as ours
      const el = document.elementFromPoint(pos.x, pos.y);
      return !!el && container.contains(el);
    };

    const stopTauriDrop = listenFolderDrop({
      onEnter: (pos) => {
        overlay.style.opacity = isUnderCursor(pos) ? '1' : '0';
      },
      onLeave: () => {
        overlay.style.opacity = '0';
      },
      onDrop: (paths, pos) => {
        if (!isUnderCursor(pos)) return;
        const escaped = paths.map(p => this.shellEscape(p)).join(' ');
        this.writeToTerminal(sessionId, escaped);
        instance.term.focus();
      },
