...
type NavItem = { to: string; icon: typeof LayoutDashboard; label: string; featured?: boolean };
type NavGroup = { items: NavItem[] };

const navGroups: NavGroup[] = [
  {
    items: [
      { to: '/dashboards', icon: LayoutDashboard, label: 'Dashboard', featured: true },
    ],
  },
  {
    items: [
            ...
            <div className="space-y-0.5">
              {group.items.map((item) => (
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-3 rounded-lg text-sm font-medium transition-all duration-150 ${
                      ...
                      isActive
                        ...
                        : item.featured
                          ? 'text-text-1 hover:text-accent-text hover:bg-accent-primary/10'
                ...
                >
                  ...
                  {!collapsed && (
                    <span className={item.featured ? 'font-semibold' : ''}>{item.label}</span>
                  )}
                </NavLink>
              ))}
            </div>
      ...
      <div className="border-t border-border-0">
        ...
        {!collapsed && (
          <div className="px-4 pb-3 pt-0">
            <p className="text-[10px] text-text-3" aria-hidden="true">&copy; OpenPaw &middot; Agentic Factory</p>
          </div>
        )}
      </div>
    </aside>
