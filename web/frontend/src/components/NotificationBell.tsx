...
export function NotificationBell() {
  ...
  return (
    <div className="relative" ref={panelRef}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 w-80 max-h-96 rounded-lg border border-border-0 bg-surface-1 shadow-xl z-50 flex flex-col overflow-hidden">
          <div className="flex items-center justify-between px-3 py-2 border-b border-border-0">
            <span className="text-xs font-semibold text-text-0">Notifications</span>
            {unreadCount > 0 && (
              <button
                onClick={() => markAllRead()}
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map(n => (
                <div
                  ...
                  className={`group flex items-start gap-2.5 px-3 py-2.5 border-b border-border-0 last:border-0 cursor-pointer transition-colors ${
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-1">
                      <p className={`text-xs leading-snug truncate ${n.read ? 'text-text-2' : 'text-text-0 font-medium'}`} title={n.title}>
                        {n.title}
                      </p>
                      <button
                        onClick={(e) => { e.stopPropagation(); dismiss(n.id); }}
                        aria-label="Dismiss notification"
                        className="opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus:opacity-100 flex-shrink-0 p-0.5 text-text-3 hover:text-text-1 transition-all cursor-pointer"
                      >
                        <X className="w-3 h-3" aria-hidden="true" />
                      </button>
                    </div>
                    {n.body && (
                      <p className="text-[11px] text-text-3 mt-0.5 line-clamp-2">{n.body}</p>
                    )}
                    <div className="flex items-center gap-2 mt-1">
                      {n.source_agent_slug && (
                        <span className="text-[10px] text-text-3 bg-surface-2 px-1.5 py-0.5 rounded">{n.source_agent_slug}</span>
                      )}
