...
const libraryTabs: { key: LibraryTab; label: string; icon: typeof Wrench }[] = [
  ...
  { key: 'tools', label: 'Services', icon: Wrench },
    ...
    <Modal open={open} onClose={onClose} title={skill.name} size="md">
      <div className="space-y-3">
        ...
        {skill.required_tools && skill.required_tools.length > 0 && (
          <div className="rounded-lg bg-surface-2 p-3">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-text-3 mb-1.5">Required Services</p>
            <div className="flex flex-wrap gap-1.5">
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <EmptyState icon={<Package className="w-8 h-8" />} title="No services found" description="Try a different search or category filter." />
      ) : (
