...
import { Bot, Plus, Upload, X, Shield, Library } from 'lucide-react';
...
function CreateAgentModal({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: (role: AgentRole) => void }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      onCreated(role);
      onClose();
      setName(''); setDescription(''); setSystemPrompt(''); setAvatarPath(PRESET_AVATARS[0]);
  ...
  return (
    <Modal open={open} onClose={onClose} title="Create Agent" size="md">
      <div className="space-y-5">
        <div>
          <label className="block text-xs font-medium text-text-1 mb-2">Avatar</label>
            ...
            {PRESET_AVATARS.map(path => (
              <button
                ...
                className={`w-14 h-14 rounded-xl overflow-hidden border-2 transition-all cursor-pointer ${
                  avatarPath === path ? 'border-accent-primary ring-2 ring-accent-primary/30' : 'border-border-1 hover:border-border-0'
                }`}
              >
                <img src={path} alt="" className="w-full h-full" />
              </button>
            ))}
            <label className={`w-14 h-14 rounded-xl border-2 border-dashed flex items-center justify-center cursor-pointer transition-all ${
              !PRESET_AVATARS.includes(avatarPath) ? 'border-accent-primary bg-accent-muted' : 'border-border-1 hover:border-border-0'
            }`}>
              {uploading ? (
                ...
                <img src={avatarPath} alt="Custom" className="w-full h-full rounded-xl" />
              ) : (
        ...
        <Input label="Name" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Atlas" />
        <Input label="Description" value={description} onChange={e => setDescription(e.target.value)} placeholder="e.g. Project Manager" />
        <Textarea label="System Prompt" value={systemPrompt} onChange={e => setSystemPrompt(e.target.value)} placeholder="You are a helpful AI assistant that..." rows={4} />

        <div className="flex justify-end gap-2 pt-2">
          <Button variant="ghost" onClick={onClose}>Cancel</Button>
          <Button onClick={handleCreate} loading={saving} disabled={!name.trim()} icon={<Plus className="w-4 h-4" />}>Create Agent</Button>
        </div>
      </div>
    </Modal>
  );
...
export function Agents() {
  ...
  const filteredRoles = roles.filter(role => {
    if (role.slug === 'builder') return false;
  ...
  const paginatedRoles = filteredRoles.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE);

  const toggleRole = async (slug: string, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      const result = await api.put<{ slug: string; enabled: boolean }>(`/agent-roles/${slug}/toggle`);
      setRoles(prev => prev.map(r => r.slug === slug ? { ...r, enabled: result.enabled } : r));
  ...
  const deleteRole = async (slug: string, name: string, e: React.MouseEvent) => {
    e.stopPropagation();
        ...
        {loading ? (
          ...
          <>
            <div className="flex items-center gap-3 mb-4">
              <SearchBar value={search} onChange={handleSearch} placeholder="Search agents..." className="flex-1" />
              <ViewToggle view={view} onViewChange={setView} />
              <Button size="sm" onClick={() => setCreateOpen(true)} icon={<Plus className="w-4 h-4" />}>Add Agent</Button>
            </div>
            ...
            {filteredRoles.length === 0 ? (
              <EmptyState
                ...
                description={search.trim() ? 'Try a different search term.' : 'Install from the Library page or create a custom agent.'}
              />
            ) : view === 'grid' ? (
              <>
                      ...
                      <div className="relative z-10 flex flex-col justify-between h-full p-6">
                        <div className="flex items-start justify-between">
                          <div className="relative">
                            <img src="/gateway-avatar.png" alt="Pounce" className="w-16 h-16 rounded-2xl shadow-lg" />
                            <div className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-accent-primary flex items-center justify-center ring-2 ring-black/40">
                        ...
                        <div className="mt-auto">
                          <div className="flex items-center gap-2">
                            <p className="text-xl font-bold text-white">Pounce</p>
                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-accent-primary/20 text-accent-primary font-medium">Gateway</span>
                          </div>
                          <p className="text-[15px] text-white/60 mt-1.5 line-clamp-2 leading-snug">Routes conversations, builds tools, dashboards, and agents</p>
                        </div>
                      </div>
                    </div>
                  </Card>
                    ...
                    <Card key={role.slug} padding={false} hover className="overflow-hidden border-border-0!" onClick={() => navigate(`/agents/${role.slug}`)}>
                      <div className="relative h-48">
                        ...
                        <div className="relative z-10 flex flex-col justify-between h-full p-6">
                          ...
                          <div className="mt-auto">
                            <p className="text-xl font-bold text-white">{role.name}</p>
                            <p className="text-[15px] text-white/60 mt-1.5 line-clamp-2 leading-snug">{role.description}</p>
                          </div>
                        </div>
                      </div>
                    </Card>
                  ...
                  <Card hover onClick={() => navigate('/agents/gateway')}>
                    <div className="flex items-center gap-3 md:gap-4">
                      <div className="relative flex-shrink-0">
                        <img src="/gateway-avatar.png" alt="Pounce" className="w-10 h-10 md:w-12 md:h-12 rounded-xl" />
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-semibold text-text-0 truncate">Pounce</p>
                  ...
                  {paginatedRoles.map(role => (
                    <Card key={role.slug} hover onClick={() => navigate(`/agents/${role.slug}`)}>
                      <div className="flex items-center gap-3 md:gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <p className="text-sm font-semibold text-text-0 truncate">{role.name}</p>
                            {role.library_slug && (
                        ...
                        <div className="flex items-center gap-1.5 md:gap-2 flex-shrink-0">
                          {!role.is_preset && (
                            <button
                              onClick={(e) => deleteRole(role.slug, role.name, e)}
