...
export function AgentEdit() {
  ...
  }, [slug, toast, navigate, loadFiles, loadMemory, loadSkills, loadTools]);
  ...
  const handlePromptKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Tab') {
      ...
      const setter = activeTab === 'legacy' ? setSystemPrompt : (v: string) => {
        const tab = FILE_TABS.find(t => t.key === activeTab);
        if (tab) handleFileChange(tab.filename, v);
      };
      const currentVal = activeTab === 'legacy' ? systemPrompt : (files[FILE_TABS.find(t => t.key === activeTab)?.filename || ''] || '');
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-5xl mx-auto p-4 md:p-6">
          <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-5 md:gap-6">
            ...
            {/* Right column — tabbed file editor or legacy prompt */}
              ...
              {!isInitialized ? (
                <>
                  <Card className="mb-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-text-0">Identity System</p>
                        <p className="text-xs text-text-3 mt-0.5">
                          Initialize the identity file system to unlock memory, skills, and self-editing.
                        </p>
                      </div>
                      <Button size="sm" onClick={handleInitIdentity} loading={initializing} icon={<Sparkles className="w-3.5 h-3.5" />}>
                        Initialize
                      </Button>
                    </div>
                  </Card>
                  <Card className="flex flex-col flex-1">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-xs font-semibold uppercase tracking-wider text-text-3">System Prompt</h3>
                      </div>
                      <span className="text-[11px] text-text-3 font-mono tabular-nums">
                        {systemPrompt.split('\n').length} lines · {systemPrompt.length} chars
                      </span>
                    </div>
                    <div className="relative flex-1 min-h-[400px]">
                      <textarea
                        ...
                        placeholder="Define this agent's personality, expertise, and behavior guidelines..."
                ...
                <Card className="flex flex-col flex-1">
                  ...
                  <div role="tablist" className="flex items-center gap-1 mb-4 overflow-x-auto pb-1 -mx-1 px-1">
                    ...
                    >
                      ...
                      {agentSkillList.length > 0 && (
                        <span className="ml-1 text-[10px] bg-surface-3 text-text-3 px-1.5 rounded-full">{agentSkillList.length}</span>
                      )}
                    </button>
                    <button
                      role="tab"
                      id="tab-agent-tools"
                      aria-selected={activeTab === 'agent-tools'}
                      aria-controls="tabpanel-agent-tools"
                      onClick={() => { setActiveTab('agent-tools'); if (slug) loadTools(slug); }}
                      className={`px-3 py-1.5 text-xs font-medium rounded-lg whitespace-nowrap transition-colors cursor-pointer ${
                  ...
                  {activeTab === 'memory' ? (
                    ...
                    <div role="tabpanel" id="tabpanel-skills" aria-labelledby="tab-skills" className="flex flex-col flex-1">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <Sparkles className="w-4 h-4 text-accent-primary" />
                      ...
                      {showSkillPicker && availableGlobalSkills.length > 0 && (
                        <div className="mb-4 p-3 rounded-lg border border-border-1 bg-surface-0">
                      ...
                      {agentSkillList.length === 0 ? (
                        ...
                        <div className="space-y-2">
                          {agentSkillList.map(skill => (
                            <div key={skill.name} className="flex items-center gap-3 p-3 rounded-lg border border-border-1 bg-surface-0">
                    ...
                    <div role="tabpanel" id="tabpanel-agent-tools" aria-labelledby="tab-agent-tools" className="flex flex-col flex-1">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                      ...
                      {agentTools.length === 0 ? (
                        <div className="flex-1 flex items-center justify-center text-sm text-text-3">No tools assigned. Grant access to tools from the library.</div>
                      ) : (
                        <div className="space-y-2">
                          {agentTools.map(tool => (
                            <div key={`${tool.id}-${tool.access_type}`} className="flex items-center gap-3 p-3 rounded-lg border border-border-1 bg-surface-0">
