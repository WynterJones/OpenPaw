...
function OpenClawCard() {
  ...
  const refresh = useCallback(() => {
    api
      .get<{ available: boolean; version: string; imported: number }>(
        "/settings/openclaw",
      )
      .then(setStatus)
      .catch(() => {});
  ...
  const sync = async () => {
    ...
    try {
      const res = await api.post<{ imported: number }>(
        "/settings/openclaw/sync",
      );
      toast("success", `Imported ${res.imported} OpenClaw agents`);
      refresh();
    } catch (err) {
      toast(
        "error",
        err instanceof Error ? err.message : "OpenClaw sync failed",
      );
    } finally {
      setBusy(false);
    }
  ...
  const remove = async () => {
    setBusy(true);
    try {
      const res = await api.delete<{ removed: number }>("/settings/openclaw");
      toast("success", `Removed ${res.removed} OpenClaw agents`);
      refresh();
    } catch (err) {
      toast(
        "error",
        err instanceof Error ? err.message : "Failed to remove agents",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <Card>
      ...
      <p className="text-xs text-text-3 mb-4">
        OpenClaw detected ({status.version}). Import its agents to chat with
        them here — same brains, memory, and sessions you talk to on Telegram.
        Replies are generated by OpenClaw, not OpenPaw.
      </p>
      <div className="flex items-center gap-3 flex-wrap">
        <Button onClick={sync} loading={busy} icon={<Bot className="w-4 h-4" />}>
          {status.imported > 0 ? "Re-sync Agents" : "Import Agents"}
        </Button>
        {status.imported > 0 && (
          <>
            <span className="text-xs text-text-2">
              {status.imported} agent{status.imported === 1 ? "" : "s"} imported
...
function DesignTab() {
  const { accent, config, bgImage, showMascot, setPreviewBg, saveAll, resetConfig } =
    useDesign();
  const { toast } = useToast();
  ...
  // Live preview background. A thumbnail cannot show whether an image fights
  // with the UI on top of it, so the pick is applied to the whole app at once
  // — as a preview, not a save. Cleared on unmount, so leaving without saving
  // reverts to whatever is stored.
  useEffect(() => {
    setPreviewBg(localBg);
    return () => setPreviewBg(null);
  }, [localBg, setPreviewBg]);
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {localBg &&
              ...
              // Generated backgrounds also live under /api/, but they have
              // their own section above — don't claim them as an upload.
              !localBg.startsWith("/api/v1/backgrounds/") && (
...
function CompanionTab() {
  const { toast } = useToast();
  // The management list shows every companion, including ones scoped to
  // another workspace — otherwise re-scoping one would make it disappear.
  const { library: characters } = useCompanionStore();
